...
export default function SignUp({ redirectTo }: { redirectTo: string }) {
  ...
  const postEmail = async ({
    ...
    let json: { data?: unknown; error?: { message?: string; code?: string } | null };
    try {
      json = raw ? JSON.parse(raw) : { error: { message: "Empty response from server." } };
    } catch {
      json = { error: { message: "Invalid response from server." } };
    }
